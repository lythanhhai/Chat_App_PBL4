        ...
        public void XLNhan()
        {
            try
            {
                while (true)
                {
                    ...
                    //data += encoding.GetString(bytesToRead);
                    //sb.Append(encoding.GetString(bytesToRead));
                    //}
                    //while (bytesRead != 0);
                    //data = data.ToString().Trim();
                    MessageBox.Show("a");
                    //MessageBox.Show(encoding.GetString(bytesToRead));
                    //MessageBox.Show(data);

                    //string data = encoding.GetString(message);
                    //ns = new NetworkStream(socket);
                    //MemoryStream imagestream = new MemoryStream(encoding.GetBytes(data));
                    //Image image1 = Image.FromStream(ns);
